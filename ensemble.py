...
if __name__ == "__main__":
    ...
    parser.add_argument('--joint-dir',
                        ...
                        ,default='/18355272020/SkeletonGCL-main/work_dir/ntu60/xsub/ctrgcn_joint')
    parser.add_argument('--bone-dir',
                        help='Directory containing "epoch1_test_score.pkl" for bone eval results'
                        ,default='/18355272020/SkeletonGCL-main/work_dir/ntu60/xsub/ctrgcn_bone')
    parser.add_argument('--joint-motion-dir'
                        ,default='/18355272020/SkeletonGCL-main/work_dir/ntu60/xsub/ctrgcn_joint_motion')
    parser.add_argument('--bone-motion-dir'
                        ,default='/18355272020/SkeletonGCL-main/work_dir/ntu60/xsub/ctrgcn_bone_motion')
    ...
    if 'UCLA' in arg.dataset:
        ...
        if 'xsub' in arg.dataset:
            npz_data = np.load('/18355272020/CTR-GCN-main-cross/data/' + 'ntu120/' + 'NTU120_CSub.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xset' in arg.dataset:
            npz_data = np.load('/18355272020/CTR-GCN-main-cross/data/' + 'ntu120/' + 'NTU120_CSet.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    elif 'ntu' in arg.dataset:
        if 'xsub' in arg.dataset:
            npz_data = np.load('/18355272020/CTR-GCN-main-cross/data/' + 'ntu/' + 'NTU60_CS.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
        elif 'xview' in arg.dataset:
            npz_data = np.load('/18355272020/CTR-GCN-main-cross/data/' + 'ntu/' + 'NTU60_CV.npz')
            label = np.where(npz_data['y_test'] > 0)[1]
    else:
