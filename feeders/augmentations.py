...
def spatial_masking(input_data):
    """
    对输入的空间特征图进行空间掩码操作，以掩盖部分关键点信息。

    Args:
        input_data (np.ndarray): 输入的空间特征图，形状为 (batch_size, channels, num_joints, height, width)。

    Returns:
        np.ndarray: 经过空间掩码操作后的空间特征图，形状与输入相同，但部分关键点的信息被掩盖为0。
